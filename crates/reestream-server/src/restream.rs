...
impl RestreamStore {
    ...
    fn from_parts(
        ...
        let state_key = state_path
            .as_ref()
            .and_then(|path| persistence::load_or_create_state_key(path));
        let (mut data, legacy_sensitive) = persistence::load_state(state_path.as_deref());
        let mut migrated_legacy = false;
        if let (Some(path), Some(key)) = (state_path.as_deref(), state_key.as_deref()) {
            if let Some(sensitive) = persistence::load_sensitive_state(path, key) {
                persistence::apply_sensitive_state(&mut data, sensitive);
                ...
                persistence::apply_sensitive_state(&mut data, legacy_sensitive);
        ...
        if let (Some(snapshot), Some(path), Some(key)) = (
            ...
            match serde_json::to_vec_pretty(&snapshot)
                .ok()
                .zip(persistence::encrypted_sensitive_state(&snapshot, key))
            {
                Some((public, sensitive)) => {
                    if let Err(error) =
                        persistence::write_state_files(path, &public, Some(&sensitive))
                    {
    ...
    async fn persist(&self, data: &RestreamData) {
        ...
        let sensitive = self
            .state_key
            .as_deref()
            .and_then(|key| persistence::encrypted_sensitive_state(data, key));
        let path = path.clone();
        let result = tokio::task::spawn_blocking(move || {
            persistence::write_state_files(&path, &public, sensitive.as_deref())
        })
        .await
