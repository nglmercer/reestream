  ...
  return (
    <div class="modal-backdrop" onClick={onClose}>
      <form class="modal-card" onSubmit={submit} onClick={(event) => event.stopPropagation()}>
        <div class="modal-header"><div><span class="eyebrow">{t('home.newStream')}</span><h2>{t('home.createTitle')}</h2></div><button type="button" class="icon-button" onClick={onClose}><Icon name="close" size={18} /></button></div>
        ...
        <div class="field-label">{t('home.pairChannels')}<div class="channel-picker">
          {channels.length === 0 ? <span class="field-help">{t('home.noChannels')}</span> : channels.map((channel) => <button type="button" key={channel.id} class={destinationIds.includes(channel.id) ? 'channel-picker-item is-selected' : 'channel-picker-item'} onClick={() => toggleChannel(channel.id)}><span class="channel-avatar channel-avatar--0">{channel.displayName.slice(0, 1).toUpperCase()}</span><span>{channel.displayName}</span><span class="picker-check">{destinationIds.includes(channel.id) ? '✓' : ''}</span></button>)}
        </div></div>
